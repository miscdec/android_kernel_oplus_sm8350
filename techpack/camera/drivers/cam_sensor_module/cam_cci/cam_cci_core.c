	...
	do {
		...
		if (read_cfg->num_byte >= CCI_READ_MAX) {
			...
			for (retry = 0; retry < 5; retry++) {
				...
				CAM_ERR(CAM_CCI, "cci wait timeout retry:%d", retry);
			}
#else
			rc = cam_cci_burst_read(sd, c_ctrl);
#endif
	...
	case MSM_CCI_INIT:
		...
		for (i = 0; i < MASTER_MAX; i++)
			for (j = 0; j < NUM_QUEUES; j++)
				mutex_lock(&cci_dev->cci_master_info[i].mutex_q[j]);

		rc = cam_cci_init(sd, cci_ctrl);

		for (i = MASTER_MAX - 1; i >= 0; i--)
			for (j = NUM_QUEUES - 1; j >= 0; j--)
				mutex_unlock(&cci_dev->cci_master_info[i].mutex_q[j]);
