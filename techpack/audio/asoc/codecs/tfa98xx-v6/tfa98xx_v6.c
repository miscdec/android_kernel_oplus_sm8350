...
static const struct soc_enum ftm_spk_rev_enum = SOC_ENUM_SINGLE_EXT(2,
		ftm_spk_rev_text);
		...
		int *speakerImpedance)
{
	...
	error = tfaRunSpeakerBoost_v6(tfa, 1,
				      profile); /* No force coldstart (with profile 0) */
...
static struct snd_soc_codec *snd_soc_kcontrol_codec(struct snd_kcontrol
		*kcontrol)
{
			      ...
			      struct snd_ctl_elem_info *uinfo)
{
	...
	uinfo->value.integer.max = max(0, tfacont_get_max_vstep_v6(tfa98xx->tfa,
				       profile) - 1);
	pr_info("vsteps count: %d [prof=%d]\n", tfacont_get_max_vstep_v6(tfa98xx->tfa,
			profile),
		profile);
	return 0;
}
			       ...
			       struct snd_ctl_elem_value *ucontrol)
{
	...
	list_for_each_entry(tfa98xx, &tfa98xx_device_list, list) {
		mutex_lock(&tfa98xx->dsp_lock);
		ucontrol->value.integer.value[tfa98xx->tfa->dev_idx] = tfa_dev_mtp_get(
					tfa98xx->tfa, TFA_MTP_RE25_PRIM);
		mutex_unlock(&tfa98xx->dsp_lock);
	}
	mutex_unlock(&tfa98xx_mutex);
...
static void tfa98xx_interrupt(struct work_struct *work)
{
	...
	if (tfa98xx->flags & TFA98XX_FLAG_REMOVE_PLOP_NOISE) {
		...
		mutex_lock(&tfa98xx->dsp_lock);
		start_triggered = tfa_plop_noise_interrupt(tfa98xx->tfa, tfa98xx->profile,
				  tfa98xx->vstep);
	...
	if (tfa98xx->flags & TFA98XX_FLAG_LP_MODES)
		tfa_lp_mode_interrupt(tfa98xx->tfa); /* TFA98XX_FLAG_LP_MODES */
			   ...
			   struct snd_soc_dai *dai)
{
	...
	/* copy profile name into basename until the . */
	get_profile_basename(basename, tfa_cont_profile_name(tfa98xx,
			     tfa98xx->profile));
	len = strlen(basename);
			     ...
			     const struct i2c_device_id *id)
{
	...
	tfa98xx_debugfs = debugfs_create_file(TFA98XX_DEBUG_FS_NAME,
					      S_IFREG | S_IRUGO | S_IWUSR, NULL, (void *)TFA98XX_DEBUG_FS_NAME,
					      &tfa98xx_debug_ops);
#else
	proc_create_data(TFA98XX_DEBUG_FS_NAME,
			 S_IFREG | S_IRUGO | S_IWUSR, NULL, &tfa98xx_debug_ops,
...
static int __init tfa98xx_i2c_init(void)
{
	...
	tfa98xx_cache =
		kmem_cache_create("tfa98xx_cache", /* Cache name /proc/slabinfo */
