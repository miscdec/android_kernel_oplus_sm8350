...
static void p9418_enable_func(struct work_struct *work)
{
	...
	if ((chip->rx_soc > chip->soc_threshould) || \
		...
		(false != chip->is_power_on)) {			/* check second time */
		chg_err("enable failed for (%d %d %d).\n", chip->rx_soc, chip->pen_status, chip->is_power_on);
		goto OUT;
	}
...
static ssize_t rx_soc_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if ((chip->rx_soc <= chip->soc_threshould) && \
		(chip->pen_status == PEN_STATUS_NEAR) && \
		(false == chip->is_power_on)) {			/* check first time */
...
static ssize_t soc_thd_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct oplus_p9418_ic *chip = NULL;
	...
	return sprintf(buf, "%d, %d\n", chip->soc_threshould, chip->power_enable_times);
}

static ssize_t soc_thd_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int val = 0;
	struct oplus_p9418_ic *chip = NULL;
