...
int tp_register_irq_func(struct touchpanel_data *ts)
{
	...
	TP_INFO(ts->tp_index, "%s ts->irq is %d\n", __func__, ts->irq);
	ts->irq_flags = IRQ_TYPE_LEVEL_HIGH | IRQF_ONESHOT;
...
#elif IS_ENABLED(CONFIG_QCOM_PANEL_EVENT_NOTIFIER)
		if (ts->active_panel) {
			cookie = panel_event_notifier_register(GET_INDEX(cur_tp_index),
					GET_TOUCH(cur_tp_index), ts->active_panel,
...
#ifdef CONFIG_TOUCHPANEL_TRUSTED_TOUCH
		ts->bus_type_tvm = BUS_TYPE_SPI;
#endif
	}
	else {
		ts->irq = ts->client->irq;
#ifdef CONFIG_TOUCHPANEL_TRUSTED_TOUCH
		ts->bus_type_tvm = BUS_TYPE_I2C;
#endif
	}
